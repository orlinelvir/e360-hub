...
export default function GHLOnboardingWizardModal({
  ...
  const handleNextStep2 = () => {
    if (!apiKey.trim()) {
      setErrorMsg("Por favor ingresa tu Token de Integración (API Key) para completar.");
      return;
    }
    setErrorMsg("");
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg md:text-xl font-extrabold text-white">
                  Vinculación Guiada con StartPoint CRM
                </h2>
                <p className="text-xs text-gray-400">
                  Configura tu subcuenta personal en 2 sencillos pasos paso a paso.
                </p>
              </div>
            </div>
          ...
          {isAlreadyConnected ? (
            <div className="space-y-6 text-center py-4">
              ...
              <div>
                ...
                <h3 className="text-xl font-extrabold text-white">Tu Subcuenta StartPoint ya está vinculada</h3>
                <p className="text-xs text-gray-400 mt-2 max-w-md mx-auto leading-relaxed">
                  Tu Location ID <strong className="text-cyan-400 font-mono">{currentLocationId || locationId}</strong> y tu llave API están configurados correctamente y transmitiendo datos sin conflictos.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-4 border-t border-gray-800">
                ...
                >
                  Cambiar o Re-configurar Llaves
                </button>
              </div>
            </div>
          ) : (
              ...
              <div className="bg-[#05101F] border border-gray-800 rounded-2xl p-5 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] uppercase font-bold text-cyan-400 tracking-wider bg-cyan-500/10 px-2.5 py-1 rounded-md border border-cyan-500/20">
                    Paso 1 de 2: Copiar tu Location ID
                  </span>
                  <a
                ...
                <div className="pt-2">
                  ...
                  {showHelpGuide && (
                    <div className="mt-3 bg-black/60 border border-cyan-500/30 rounded-xl p-3 text-xs text-gray-300 space-y-2">
                      ...
                      <p>💡 Cada broker tiene su propio Location ID único para que nadie más pueda ver sus clientes.</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* PASO 2: API KEY / PRIVATE INTEGRATION */}
          {step === 2 && (
            <div className="space-y-6">
              <div className="bg-[#05101F] border border-gray-800 rounded-2xl p-5 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] uppercase font-bold text-cyan-400 tracking-wider bg-cyan-500/10 px-2.5 py-1 rounded-md border border-cyan-500/20">
                    Paso 2 de 2: Generar tu Private Integration Key
                  </span>
                  <a
                    href="https://app.startpoint.biz"
                    target="_blank"
                ...
                <h3 className="text-sm font-bold text-white">
                  ¿Cómo creo mi llave de integración en StartPoint?
                </h3>

                <ol className="text-xs text-gray-300 space-y-2 list-decimal list-inside leading-relaxed pl-1">
                  <li>Dentro de tu subcuenta en StartPoint, ve a <strong>Settings (Configuración) ⚙️</strong>.</li>
                  <li>Haz clic en <strong>Private Integrations (Integraciones Privadas)</strong>.</li>
                  <li>Haz clic en <strong>+ Create Private Integration (+ Crear Integración)</strong>.</li>
                  <li>En la sección <strong>Scopes (Permisos)</strong>, marca las casillas de <strong>Contacts</strong> y <strong>Opportunities</strong> (Read y Write).</li>
                  <li>Guarda y copia la llave que empieza por <strong className="text-cyan-400 font-mono">pit-xxxx</strong>.</li>
                </ol>
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-300 uppercase tracking-wider mb-2">
                  Ingresa o Pega aquí tu Token (pit-xxxx):
                </label>
                <div className="relative">
          ...
          {step === 3 && (
            <div className="text-center py-8 space-y-6">
              ...
              <div>
                <h3 className="text-2xl font-extrabold text-white">¡Conexión Exitosa con StartPoint!</h3>
                <p className="text-xs text-gray-400 mt-2 max-w-md mx-auto leading-relaxed">
                  Tu subcuenta de CRM ha sido vinculada correctamente. Todos los clientes y oportunidades se sincronizarán en vivo en tu panel de <strong>Mis Clientes</strong>.
                </p>
              </div>
